document.addEventListener('DOMContentLoaded', () => { // ¡Este es el ÚNICO DOMContentLoaded!
    ...
    if (submitAct1Btn) { // Comprobación defensiva
        submitAct1Btn.addEventListener('click', () => {
            ...
            fetch('/users/actividades-4', { method: 'post', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ activity: 'act_1', score: score }) });
        });
    }
    ...
    if (submitAct2Btn) { // Comprobación defensiva
        submitAct2Btn.addEventListener('click', () => {
            ...
            fetch('/users/actividades-4', { method: 'post', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ activity: 'act_2', score: score }) });
        });
    }
    ...
    if (submitAct3Btn) { // Comprobación defensiva
        submitAct3Btn.addEventListener('click', () => {
            ...
            fetch('/users/actividades-4', { method: 'post', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ activity: 'act_3', score: score }) });
        });
    }
    ...
    if (submitQuizFinalBtn) { // Comprobación defensiva
        submitQuizFinalBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch('/users/quiz-4', {
                    method: 'post',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ quiz: 'quiz_4', score: score })
                });
                const data = await response.json();
                if (response.ok) {
                    if (quizOverallFeedbackDiv) quizOverallFeedbackDiv.innerHTML += `<br>${data.message}`;
                    if (data.unlockedNextModule) {
                        alert("¡Felicidades! Has aprobado el Módulo 4 y la prueba final ha sido desbloqueada.");
                    }
                } else {
